...
class TaskConfig:
    def __init__(self, dataset_script, columns, batch_size, eval_batch_size, metrics, task_type="cls", bins_num=None):
...
def define_dataset_config(fine_tune_task=None, batch_size=64, pair_flag=False):
    ...
    datasets_config = {
        ...
        Task.glue_sts_b: TaskConfig(dataset_script="./datasets/glue_sts_b.py", columns=task_columns,
                                    batch_size=batch_size, eval_batch_size=batch_size * 2,
                                    metrics=[pearsonr, spearmanr], task_type="res", bins_num = 5),
        Task.claire_v2: TaskConfig(dataset_script="./datasets/claire_v2.py", columns=task_columns,
                                   batch_size=batch_size, eval_batch_size=batch_size * 2,
                                   metrics=[pearsonr, spearmanr], task_type="res", bins_num = 4),
...
def define_tasks_config(datasets_config, dataset_percentage=1.0, cache_dir="./datasets/cache/", class_weights_flag=False, noise_rate=0.0):
    tasks_config = {}
    for id, (task, task_config) in enumerate(datasets_config.items()):
        ...
        if noise_rate > 0:
            ...
            train_label = np.array(train_dataset['label']).squeeze()
            num_noise = int(len_dataset * noise_rate)
            noise_idx = np.random.choice(np.arange(len_dataset), num_noise, False)
            noise_label = np.random.choice(np.arange(num_classes), num_noise, True)
            train_label[noise_idx] = noise_label
            train_dataset = train_dataset.remove_columns('label').add_column('label', train_label)
        # convert res label to cls label
        if task_config.task_type == 'res':
            train_label = np.array(train_dataset['label']).squeeze()
            _, bins = np.histogram(train_label, bins=task_config.bins_num)
            bins[-1] = bins[-1] + 1e-7 # 容纳右边界
